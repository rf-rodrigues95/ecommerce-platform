  ...
  export default function ProductsTable({actions}: Props) {
    ...
    const handleAddToCart = async (id: number, quantity: number) => {
      try {
        await actions.addToCart(id, quantity);
      } catch (err) {
                    ...
                    {user?.role === UserRole.ADMIN ? (
                        <>
                          ...
                          <Button 
                            ...
                            className="px-2 py-1 text-sm rounded-md border border-gray-300 hover:bg-green-400 text-white cursor-pointer">
                              Add to Cart
                          </Button>
                        ...
                        <div className="flex justify-center">
                          <Button 
                            onClick={() => handleAddToCart(p.id, 1)}
                            className="px-4 py-2 w-[260px] text-sm rounded-md border border-gray-300 hover:bg-green-400 text-white cursor-pointer">
                            Add to Cart
                          </Button>
                        </div>
                      )}
